    ...
    return <div className="space-y-8">
        ...
        {selected && <Section title="Décision de réception">
            <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 md:grid-cols-2">
                <div><p className="text-xs font-bold uppercase tracking-wide text-slate-600">Livraison</p><p className="mt-1 font-black text-slate-900">{selected.reference}</p><p className="mt-2 text-sm text-slate-700">Client : {selected.client?.firstName} {selected.client?.lastName}</p></div>
                <div><p className="text-xs font-bold uppercase tracking-wide text-slate-600">Transport</p><p className="mt-1 text-sm text-slate-700">{selected.carrierName || "Livreur non renseigné"}</p><p className="mt-2 text-sm text-slate-600">{selected.contents || selected.type || "Contenu non renseigné"}</p></div>
            </div>
