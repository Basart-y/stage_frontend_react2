...
export default function ReceptionPage() {
    ...
    return (<div className="space-y-8">
        <PageTitle title="Réception colis" description="Enregistrez l'arrivée d'un colis au point relais."/>
        <Section title="Mode de réception">
            <div className="flex gap-4">
                <button
                    onClick={() => setMode("manual")}
                    className={`rounded-lg px-5 py-3 ${mode === "manual" ? "bg-green-600" : "bg-slate-800"}`}>
                    Réception manuelle
                </button>
                <button
                    onClick={() => setMode("qr")}
                    className={`rounded-lg px-5 py-3 ${mode === "qr" ? "bg-green-600" : "bg-slate-800"}`}>
                    Scanner QR Code
                </button>
            </div>
        ...
        {mode === "qr" && (<Section title="Scanner le QR Code">
            <div
                ...
                <div className="text-center space-y-3">
                    <div
                        className=" mx-auto flex h-20 w-20 items-center justify-center rounded-xl bg-green-600/20 text-4xl">
                        ▦
                    </div>
        ...
        <button onClick={receive} className="rounded-lg bg-green-600 px-6 py-3 font-medium hover:bg-green-500">
            Valider réception
        </button>
