...
function StatusBadge({status}) {
    const styles = {"Créée": "bg-blue-500/10 text-blue-300", "En transit": "bg-amber-500/10 text-amber-300", "Arrivé au point relais": "bg-violet-500/10 text-violet-300", "Retiré": "bg-emerald-500/10 text-emerald-300", "Retourné": "bg-red-500/10 text-red-300"};
    return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${styles[status] || "bg-slate-800 text-slate-200"}`}>{status}</span>;
}
...
export default function SuiviPage() {
    ...
    return <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-700 bg-[#111b2b] p-4 shadow-sm md:flex-row"><SearchBar value={search} onChange={setSearch} placeholder="Référence ou point relais..."/><FiltresLivraisons status={status} setStatus={setStatus}/></div>
        {error && <Alert type="error" message={error}/>} 
        {loading ? <Loading message="Chargement de vos livraisons..."/> : <TableauDonnees columns={[{key: "reference", label: "Référence", render: row => <span className="font-semibold text-slate-100">{row.reference}</span>},{key: "relayPoint", label: "Point relais"},{key: "status", label: "Statut", render: row => <StatusBadge status={row.status}/>},{key: "date", label: "Date"}]} data={filtered} emptyMessage="Aucune livraison ne correspond à votre recherche."/>}
    </div>;
}
