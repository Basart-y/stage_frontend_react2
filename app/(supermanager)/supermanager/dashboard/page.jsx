...
export default function SuperManagerDashboardPage() {
  return <section className="space-y-8">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      ...
      <Section title="Couverture géographique" description="Le périmètre France est structuré par ville et département."><div className="flex items-center gap-4 rounded-2xl bg-white p-5 text-slate-950"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10"><Map size={22}/></span><div><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-600">Réseau actif</p><p className="mt-1 text-xl font-black">95 relais</p><p className="mt-1 text-xs text-slate-600">Pilotage par périmètres géographiques.</p></div></div></Section>
    </div>
    <Section title="Accédez aux espaces" description="Ouvrez rapidement les différentes vues métier sans vous déconnecter.">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {spaces.map(({label, href, icon: Icon, description}) => <Link key={href} href={href} className="group rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-blue-500/50 hover:bg-indigo-700/[0.06]">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/10 text-blue-700 transition group-hover:bg-indigo-700 group-hover:text-white"><Icon size={19}/></span>
...
function Metric({icon: Icon, label, value}) { return <div className="rounded-2xl border border-slate-200 p-4"><Icon size={18} className="text-blue-700"/><p className="mt-4 text-xs font-bold uppercase tracking-wide text-slate-600">{label}</p><p className="mt-1 text-2xl font-black tracking-tight text-slate-950">{value}</p></div>; }
