...
export default function ManagersPage() {
    ...
    async function load(){
        setLoading(true);
        try { setManagers(await serviceSuperManager.getManagers()); }
        catch(error){ setFeedback({type:"error", message:error.message || "Impossible de charger les gestionnaires."}); }
        finally { setLoading(false); }
    }
    useEffect(()=>{load();},[]);

    const activeCount = useMemo(()=>managers.filter(m=>m.statutCompte==='actif').length,[managers]);
    ...
    return <div className="space-y-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <button onClick={()=>setOpen(v=>!v)} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-500">{open?"Fermer":"Créer un accès gestionnaire"}</button>
        </div>
        <div className="grid gap-4 sm:grid-cols-3"><Summary label="Total" value={managers.length}/><Summary label="Actifs" value={activeCount}/><Summary label="À activer" value={managers.filter(m=>m.statutCompte==='invite').length}/></div>
        {feedback&&<Alert type={feedback.type} message={feedback.message}/>} 
        {open&&<form onSubmit={submit} className="grid gap-4 rounded-2xl border border-slate-700 bg-[#111b2b] p-5 md:grid-cols-2">
            <div className="md:col-span-2"><h2 className="text-lg font-bold text-white">Nouveau Gestionnaire</h2><p className="mt-1 text-sm text-slate-400">Le périmètre est appliqué par le serveur à toutes ses listes administratives.</p></div>
            ...
            <label className="space-y-2 text-sm font-medium text-slate-300"><span>Niveau du périmètre</span><select value={form.scopeLevel} onChange={e=>setForm({...form,scopeLevel:e.target.value,scopeValue:""})} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5"><option value="ville">Ville</option><option value="departement">Département</option><option value="pays">Pays</option></select></label>
            ...
            <div className="md:col-span-2"><button disabled={busy} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy?"Création…":"Créer l’invitation"}</button></div>
            {activationUrl&&<div className="md:col-span-2 rounded-xl border border-amber-700/50 bg-amber-500/10 p-4"><p className="text-sm font-semibold text-amber-200">Lien d’activation temporaire</p><p className="mt-2 break-all text-xs text-slate-300">{activationUrl}</p><button type="button" onClick={()=>navigator.clipboard.writeText(activationUrl)} className="mt-3 text-sm font-semibold text-blue-300 underline">Copier le lien</button></div>}
        </form>}
        {loading?<Loading message="Chargement des gestionnaires…"/>:<TableauDonnees columns={[
            ...
            {key:"actions",label:"Actions",render:row=>row.statutCompte==='invite'?<span className="text-xs text-slate-500">Activation en attente</span>:<button onClick={()=>toggle(row)} className={row.statutCompte==='actif'?"font-semibold text-red-300 underline":"font-semibold text-emerald-300 underline"}>{row.statutCompte==='actif'?"Suspendre":"Réactiver"}</button>}
        ]} data={managers} emptyMessage="Aucun gestionnaire créé."/>}
    </div>;
}

function Field({label,type="text",value,onChange,required=false}){return <label className="space-y-2 text-sm font-medium text-slate-300"><span>{label}</span><input type={type} required={required} value={value} onChange={e=>onChange(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-slate-100 outline-none focus:border-blue-500"/></label>}
function Summary({label,value}){return <div className="rounded-2xl border border-slate-700 bg-[#111b2b] p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold text-white">{value}</p></div>}
