...
export const serviceSignalement={
  async getAll(filters={}){if(getAccessToken()){const q=new URLSearchParams();Object.entries(filters).forEach(([k,v])=>v!==undefined&&v!==null&&v!==""&&q.set(k,String(v)));const r=await apiRequest(`/api/v1/reports?${q.toString()}`);return {data:r.data||[],pagination:r.pagination||{}};}return {data:read(),pagination:{page:1,limit:read().length,total:read().length,hasNext:false}};},
