...
export async function createDelivery(input, actor, context = {}) {
  ...
  await writeAuditTrace({ request: context.request, eventType: 'DELIVERY_CREATED', action: 'delivery.create', actor, resourceType: 'delivery', resourceId: id, after: { status: saved.status, reference: saved.reference }, metadata: { relayPointId: saved.relayPointId } });
...
export async function transitionDelivery(id, status, comment = '', extra = {}, actor = null, context = {}) {
  ...
  await writeAuditTrace({
    request: context.request, requestId: context.requestId, eventType: 'DELIVERY_STATUS_TRANSITION',
    action: `delivery.transition.${status}`, actor, resourceType: 'delivery', resourceId: id,
    before: { status: current.status, updatedAt: current.updatedAt },
    after: { status: saved.status, updatedAt: saved.updatedAt },
    metadata: { comment, receptionSheetId: saved.receptionSheet?.id || null, handoffSheetId: saved.handoffSheet?.id || null },
  });
