...
export async function createReport(input, author) {
  ...
  const notificationBase = {
    type: 'urgence', title: input.type === 'probleme_paiement' ? 'Nouveau litige de paiement' : 'Nouveau signalement',
    message: `Un signalement ${report.id.slice(0, 8)} doit être traité.`, resourceType: 'report', resourceId: report.id,
    priority: input.type === 'probleme_paiement' ? 'high' : 'normal',
  };
  if (assignedRole === 'gestionnaire') {
    const managers = await userRepository.list({ role:'gestionnaire', statutCompte:'actif', limit:500 });
    const targets = managers.filter(manager => scopeAllows(manager.scope, report.geography));
    if (targets.length) await Promise.all(targets.map(manager => createNotification({ ...notificationBase, recipientId: manager.id })));
    else await createNotification({ ...notificationBase, audienceRole:'super_gestionnaire' });
  } else {
    await createNotification({ ...notificationBase, audienceRole: assignedRole });
  }
  return report;
}
