...
export default function RegistrationRequestsPanel({ role }) {
  ...
  return <Section title={title} description={`${count} demande(s) dans ce filtre. Le Manager voit uniquement les demandes de son périmètre ; le Super Gestionnaire peut toutes les traiter.`}>
    <div className="mb-4 flex flex-wrap gap-2">
      {[['PENDING','À valider'],['APPROVED','Validées'],['REJECTED','Refusées']].map(([value,label]) => <button key={value} type="button" onClick={() => setStatus(value)} className={`rounded-xl px-3 py-2 text-sm font-semibold ${status === value ? 'bg-blue-600 text-white' : 'border border-slate-700 bg-slate-900 text-slate-300'}`}>{label}</button>)}
    </div>
    {feedback && <div className="mb-4"><Alert type={feedback.type} message={feedback.message}/></div>}
    {loading ? <Loading message="Chargement des demandes…"/> : rows.length === 0 ? <p className="rounded-xl border border-slate-800 bg-slate-900/50 p-5 text-sm text-slate-400">Aucune demande dans ce filtre.</p> : <div className="space-y-3">{rows.map((row) => {
      const profile = row.profile || {};
      return <div key={row.id} className="rounded-2xl border border-slate-700 bg-slate-900/50 p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><p className="font-black text-slate-100">{profile.raisonSociale || profile.nom || row.email}</p><span className="rounded-full bg-slate-800 px-2.5 py-1 text-[11px] font-bold text-slate-300">{statusLabels[row.status] || row.status}</span></div>
            <p className="mt-1 text-sm text-slate-400">{row.email}</p>
            <p className="mt-1 text-xs text-slate-500">{profile.ville || '—'} · {profile.departement || '—'} · reçue le {row.createdAt ? new Date(row.createdAt).toLocaleString('fr-FR') : '—'}</p>
          </div>
          {row.status === 'PENDING' && <div className="flex gap-2"><button disabled={busyId === row.id} onClick={() => decide(row, 'REJECT')} className="rounded-xl border border-red-500/40 px-3 py-2 text-sm font-bold text-red-300 disabled:opacity-50">Refuser</button><button disabled={busyId === row.id} onClick={() => decide(row, 'APPROVE')} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-bold text-white disabled:opacity-50">Valider</button></div>}
        </div>
      </div>;
    })}</div>}
  </Section>;
