...
export default function UserHeader({userId = 1, role = "Utilisateur"}) {
    ...
    return <>
        <header className="sticky top-0 z-40 h-[76px] border-b border-slate-800 bg-[#0b1422]/95 backdrop-blur-xl">
            <div className="flex h-full items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 xl:px-10">
                <div className="min-w-0">
                    <p className="text-[10px] font-bold uppercase tracking-[.14em] text-slate-500">Espace</p>
                    <p className="mt-1 truncate text-sm font-bold text-slate-100">{role}</p>
                </div>
                <div className="flex items-center gap-2 sm:gap-3">
                    <button type="button" onClick={() => setPaletteOpen(true)} className="hidden w-64 items-center gap-2.5 rounded-xl border border-slate-700 bg-slate-900/70 px-3.5 py-2.5 text-left text-sm text-slate-400 transition hover:border-slate-600 xl:flex"><Search size={16}/><span>Rechercher...</span><span className="ml-auto inline-flex items-center gap-1 rounded-md border border-slate-700 bg-slate-800 px-1.5 py-0.5 text-[10px] font-semibold text-slate-400"><Command size={10}/>K</span></button>
                    <NotificationBell userId={userId}/>
                    <button type="button" onClick={() => { serviceAuthentification.logout(); window.location.href = "/login"; }} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-700 bg-slate-900 text-slate-400 transition hover:border-slate-600 hover:text-white" aria-label="Déconnexion"><LogOut size={17}/></button>
                </div>
            </div>
        </header>
        {paletteOpen && <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/60 px-4 pt-[14vh] backdrop-blur-sm" onMouseDown={() => setPaletteOpen(false)}>
            <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-700 bg-[#111b2b] shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-3 border-b border-slate-800 px-4"><Search size={18} className="text-slate-500"/><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Livraison, manager, point relais..." className="h-14 flex-1 bg-transparent text-sm text-slate-100 outline-none placeholder:text-slate-500"/><button onClick={() => setPaletteOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-800 hover:text-white"><X size={17}/></button></div>
                <div className="p-2">{filtered.length ? filtered.map((item) => <Link key={item.href} href={item.href} onClick={() => setPaletteOpen(false)} className="flex items-center justify-between rounded-xl px-3 py-3 text-sm font-semibold text-slate-200 hover:bg-blue-500/10 hover:text-blue-300"><span>{item.label}</span><span className="text-xs text-slate-600">↵</span></Link>) : <p className="px-3 py-7 text-center text-sm text-slate-500">Aucun résultat.</p>}</div>
            </div>
        </div>}
    </>;
}
